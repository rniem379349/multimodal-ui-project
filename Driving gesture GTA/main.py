	...
	try:
		...
		for ind,ct in enumerate(contours):#Iterate over all contours in frame
			...
			if area in range(6000,13000):#Ignore other contours that are not of hands 
				...
				cv2.putText(im,":"+Dir+str(slope)+"deg",(50,50),cv2.FONT_HERSHEY_SIMPLEX,0.5,(255,255,255),2)
				cv2.putText(im,":"+(str(distance)),(50,150),cv2.FONT_HERSHEY_SIMPLEX,0.5,(0,0,255),2)
				...
				if area>13000 and len(contours)==1:#If area is of two hands joined
					cv2.putText(im,"No Move",(50,50),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,255),4)
					Brake()
